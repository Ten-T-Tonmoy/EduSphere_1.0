...
const RequestsApproval = () => {
  ...
  const getRoleBadgeColor = (role) => {
    switch(role) {
      ...
      case 'cr': return 'bg-emerald-100 text-emerald-800 border-emerald-200';
    ...
    return (
      <div className="premium-approval-card">
        <div className="text-center py-16">
          ...
          <h3 className="text-2xl font-extrabold text-gray-800 tracking-tight">Access Restricted</h3>
          <p className="mt-2 text-gray-500 font-medium">You don't have permission to view this page.</p>
        </div>
      </div>
    );
  }
      ...
      <div className="premium-approval-card">
        ...
        <div className="border-b border-gray-200/60 mb-8">
          <nav className="-mb-px flex space-x-8 overflow-x-auto scrollbar-hide">
            {['all', 'pending', 'approved', 'rejected'].map((filterType) => (
              ...
              >
                {filterType}
                {filterType === 'pending' && requests.filter(r => r.status === 'pending').length > 0 && (
        ...
        {error && (
          <div className="mb-6 bg-red-50/80 backdrop-blur-sm border border-red-200 text-red-700 px-4 py-3 rounded-xl flex items-center justify-between shadow-sm">
            ...
            <button onClick={fetchRequests} className="text-sm text-red-700 font-bold underline hover:no-underline">Try again</button>
          </div>
        )}
        ...
        <div className="flex flex-col justify-center items-center py-20">
          ...
          <div className="text-center py-16 bg-gray-50/50 rounded-2xl border border-dashed border-gray-200">
            ...
            <h3 className="text-xl font-extrabold text-gray-800">No requests found</h3>
                ...
                <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
                  ...
                  <div className="flex-1">
                    <div className="flex items-start space-x-5">
                      ...
                      <div className="flex-1">
                        <h4 className="text-xl font-bold text-gray-900 tracking-tight">{request.user.name}</h4>
                        <p className="text-sm text-gray-500 font-medium">{request.user.email}</p>
                        ...
                        <div className="flex flex-wrap items-center gap-2.5 mt-3">
                          <span className={`badge-glow text-[11px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full border ${getRoleBadgeColor(request.user.role)}`}>
                    ...
                    <div className="mt-5 bg-white/50 backdrop-blur-sm rounded-xl p-4 border border-gray-100/80 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                      <div className="flex items-center space-x-3">
                        ...
                        <span className="text-sm text-gray-800 font-bold">{request.group.name}</span>
                      </div>
                      ...
                      <div className="flex space-x-3">
                        <button
                          ...
                          className={`flex-1 py-2.5 px-4 text-sm font-bold rounded-xl text-white transition-all shadow-md hover:-translate-y-0.5 flex items-center justify-center space-x-1 ${
                            processingId === request._id ? 'bg-emerald-400 shadow-none' : 'bg-gradient-to-r from-emerald-500 to-emerald-600 hover:shadow-emerald-200'
                          }`}
                        >
                        ...
                        <button
                          ...
                          className={`flex-1 py-2.5 px-4 text-sm font-bold rounded-xl transition-all flex items-center justify-center space-x-1 ${
                            processingId === request._id ? 'bg-gray-100 text-gray-400' : 'bg-white border-2 border-rose-100 text-rose-600 hover:bg-rose-50 hover:border-rose-200 hover:-translate-y-0.5'
                          }`}
                        >
                  ...
                  {request.status !== 'pending' && (
                    <div className="lg:w-48 flex items-center justify-center lg:justify-end h-full">
                      <span className={`badge-glow px-4 py-2 rounded-xl text-sm font-extrabold uppercase tracking-widest border shadow-sm ${getStatusBadgeColor(request.status)}`}>
                        {request.status}
                      </span>
                    </div>
                  )}
        ...
        <div className="fixed inset-0 modal-glass-overlay flex items-center justify-center p-4">
          <div className="relative modal-glass-content w-full max-w-sm p-8 animate-in fade-in zoom-in-95 duration-200">
            <div className="text-center">
              <div className={`mx-auto flex items-center justify-center h-16 w-16 rounded-2xl mb-5 shadow-inner ${
                pendingAction.status === 'approved' ? 'bg-emerald-100 border border-emerald-200' : 'bg-rose-100 border border-rose-200'
              }`}>
              ...
              <h3 className="text-2xl font-extrabold text-gray-900 mb-2 tracking-tight">
                {pendingAction.status === 'approved' ? 'Approve Member?' : 'Reject Member?'}
              </h3>
              
              <p className="text-sm text-gray-500 font-medium mb-1">
                Are you sure you want to {pendingAction.status} the request from <span className="font-bold text-gray-900">{pendingAction.userName}</span>?
              </p>
              
              <div className="bg-gray-50 rounded-lg p-3 my-4 border border-gray-100">
                <p className="text-xs text-gray-500 font-bold uppercase tracking-wider mb-1">Target Group</p>
                <p className="text-sm font-bold text-gray-800">{pendingAction.groupName}</p>
              </div>

              {pendingAction.status === 'approved' && (
                <p className="text-sm text-gray-500 font-medium mb-6">
                  Role to be assigned: <span className="inline-block px-2 py-0.5 bg-indigo-50 text-indigo-700 font-bold rounded-md ml-1">{pendingAction.roleToAssign}</span>
                </p>
              )}
