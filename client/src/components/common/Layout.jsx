...
const statusConfig = {
  connecting: { color: "bg-amber-400", ring: "ring-amber-200", label: "Connecting", text: "text-amber-700", bg: "bg-amber-50", border: "border-amber-200" },
  connected: { color: "bg-emerald-500", ring: "ring-emerald-200", label: "Connected", text: "text-emerald-700", bg: "bg-emerald-50", border: "border-emerald-200" },
  disconnected: { color: "bg-rose-500", ring: "ring-rose-200", label: "Offline", text: "text-rose-700", bg: "bg-rose-50", border: "border-rose-200" },
};
...
const ServerIndicator = ({ status }) => {
  ...
  return (
    <div className={`flex items-center gap-2 px-2 sm:px-3 py-1.5 rounded-full border ${cfg.bg} ${cfg.border} transition-all`}>
      <span className="relative flex h-2 w-2">
        {status === "connecting" && <span className={`animate-ping absolute inline-flex h-full w-full rounded-full ${cfg.color} opacity-75`} />}
        {status === "connected" && <span className={`animate-ping absolute inline-flex h-full w-full rounded-full ${cfg.color} opacity-40`} />}
        <span className={`relative inline-flex rounded-full h-2 w-2 ${cfg.color} ring-2 ${cfg.ring}`} />
      </span>
      <span className={`hidden sm:inline-block text-[10px] font-bold uppercase tracking-wide ${cfg.text}`}>
        {cfg.label}
      </span>
    </div>
  );
...
const Layout = () => {
  ...
  const canManageRequests = ["class_rep", "teacher", "admin"].includes(user?.role);

  let dynamicNavItems = [...baseNavItems];
  if (isTeacher) {
    dynamicNavItems.splice(3, 0, { to: "/dept-schedule", icon: Building2, label: "Dept. Routine" });
    dynamicNavItems.splice(4, 0, { to: "/attendance", icon: ClipboardCheck, label: "Group Attendance" });
  } else {
    dynamicNavItems.splice(3, 0, { to: "/attendance", icon: ClipboardCheck, label: "My Attendance" });
    dynamicNavItems.splice(4, 0, { to: "/chat", icon: MessageSquare, label: "Chat" });
  }
        ...
        >
          <div className="h-16 lg:h-20 flex items-center justify-between px-5 lg:px-6 border-b border-slate-100 shrink-0">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">Manager</p>
              </div>
            </div>
            <button 
              onClick={() => setSidebarOpen(false)}
          ...
          <nav className="flex-1 px-4 py-6 overflow-y-auto custom-scrollbar space-y-1">
            ...
            {dynamicNavItems.map(({ to, icon: Icon, label }) => {
              const isActive = location.pathname === to || (to !== '/' && location.pathname.startsWith(to) && to !== '/dashboard');
              return (
                <NavLink
                  key={to}
                  to={to}
                ...
                >
                  <Icon className={`w-5 h-5 transition-colors ${isActive ? 'text-white' : 'text-slate-400 group-hover:text-slate-700'}`} />
                  {label}
                </NavLink>
              );
            })}
            ...
            {canManageRequests && (
              <>
                ...
                >
                  <ClipboardCheck className={`w-5 h-5 transition-colors ${location.pathname.includes('/extra-requests') ? 'text-white' : 'text-slate-400 group-hover:text-slate-700'}`} />
                  Extra Requests
                </NavLink>
              </>
            )}
            ...
            <div className="flex items-center gap-3 sm:gap-6 ml-auto shrink-0">
              ...
              <div className="relative">
                ...
                >
                  ...
                  <div className={`absolute inset-0 bg-slate-200 rounded-full transition-all duration-300 ease-out z-0 
                  ...
                  <>
                    ...
                    <div className="absolute right-0 mt-3 w-[calc(100vw-2rem)] sm:w-80 max-w-sm bg-white rounded-2xl shadow-2xl border border-slate-100 z-50 overflow-hidden origin-top-right animate-in fade-in zoom-in-95 duration-200">
                      <div className="px-5 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                        ...
                        <span className="text-xs bg-indigo-100 text-indigo-600 px-2 py-0.5 rounded-full font-bold">{unreadCount} New</span>
                      </div>
                      ...
                      <div className="max-h-[60vh] overflow-y-auto custom-scrollbar">
                        {recentLogs.length === 0 ? (
                          <div className="p-8 text-center">
                            ...
                            <p className="text-sm font-medium text-slate-500">You have no new notifications.</p>
                          </div>
                        ) : (
                          recentLogs.map((log) => (
                            <div 
                              key={log._id}
                              onClick={() => { setNotificationsOpen(false); navigate(log.actionUrl); }}
                            ...
                            >
                              <p className="text-sm font-bold text-slate-800 flex items-center gap-1.5">
                                {log.type === 'chat' ? '💬' : log.type === 'notice' ? '📌' : '🔔'}
                                <span className="truncate">{log.title}</span>
                              </p>
                              <p className="text-xs text-slate-500 mt-1 line-clamp-2">{log.body}</p>
                            </div>
                          ))
                        )}
                      </div>
                  ...
                  <>
                    ...
                    <div className="absolute right-0 mt-3 w-64 max-w-[calc(100vw-2rem)] bg-white rounded-2xl shadow-2xl border border-slate-100 z-50 overflow-hidden origin-top-right animate-in fade-in zoom-in-95 duration-200">
                      <div className="p-5 border-b border-slate-100 bg-slate-50/50">
                        ...
                        <p className="text-xs font-bold text-slate-500 mt-1 uppercase tracking-wider truncate">{user?.role?.replace("_", " ")}</p>
                      </div>
