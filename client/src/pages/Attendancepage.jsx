...
import {
  ...
  Sparkles, User as UserIcon, LayoutGrid, FileOutput, FileSpreadsheet
} from "lucide-react";
...
const AttendancePage = () => {
  ...
  useEffect(() => {
    if (selectedGroup && selectedCourse && tab === "matrix") {
      ...
      const handleUpdate = (data) => {
        if (data.groupId === selectedGroup && data.courseId === selectedCourse) {
           api.get(`/interactive-matrix/${selectedGroup}?courseId=${selectedCourse}`).then(res => {
              setMatrixData({
                 activeSessions: res.data.activeSessions || [],
                 students: res.data.data || []
              });
  ...
  const handleExportToDocs = async () => {
    ...
    try {
      ...
      const res = await api.get(`/interactive-matrix/export/${selectedGroup}?courseId=${selectedCourse}&courseName=${course.name}`);
      
      if (res.data.success) {
        alert("Preparing data for Google Docs... Please check your connected Google Drive.");
        alert(`Export Data Prepared: "${res.data.documentTitle}"\n\nTo complete this, ensure your Google Cloud credentials are added to the server environment.`);
      }
    } catch (err) {
  ...
  const fetchToday = async () => {
    try {
      ...
      const slotsData = Array.isArray(slots.data) ? slots.data : (slots.data?.data || []);
      setTodaySlots(slotsData.filter((s) => s.dayOfWeek === today && s.status !== "cancelled"));
      
      const attendanceData = Array.isArray(att.data) ? att.data : (att.data?.data || []);
  ...
  const getSlotAttendance = (slot) => {
    ...
    const courseId = (slot.course?._id || slot.course) ? String(slot.course?._id || slot.course) : null;
    return todayAttendance.find((a) => {
      const aSlotId = (a.classSlot?._id || a.classSlot) ? String(a.classSlot?._id || a.classSlot) : null;
      const aCourseId = (a.course?._id || a.course) ? String(a.course?._id || a.course) : null;
      return (slotId && aSlotId === slotId) || (courseId && aCourseId === courseId);
    });
  };
      ...
      setMatrixData(prev => {
          const newStudents = prev.students.map(s => {
              if (s.studentId === studentId) {
                  return { ...s, attendance: { ...s.attendance, [session.key]: status } };
              }
              return s;
          });
  ...
  let combinedSessions = matrixData.activeSessions ? [...matrixData.activeSessions] : [];
  const existingTodaySessions = combinedSessions.filter(s => s.dateStr === todayStr);
  const existingCount = existingTodaySessions.length;

  if (isTeacher) {
      const requiredToday = Math.max(1 + extraClasses, existingCount === 0 ? 1 : existingCount);
        ...
        <AnimatePresence mode="wait">
                  ...
                  return (
                    <div key={slot._id} className="bg-white/80 backdrop-blur-sm p-6 rounded-[2rem] border border-white shadow-sm flex flex-col md:flex-row justify-between items-center gap-4 hover:shadow-lg transition-all">
                      ...
                      <div className="flex gap-2">
                        {att ? (
                          <span className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase border ${statusStyles[att.status.toLowerCase()] || statusStyles.attended}`}>{att.status}</span>
                        ) : (
                          <span className="px-4 py-2 rounded-xl bg-slate-50 text-slate-400 text-[10px] font-black uppercase border border-slate-200">Pending Call</span>
                        )}
                      </div>
                    </div>
                  );
          ...
          {tab === "matrix" && (
            <motion.div key="matrix" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="w-full">
              ...
              {isTeacher && selectedCourse && matrixData.students.length > 0 && (
                 <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex justify-between items-center mb-6 flex-wrap gap-4">
                     
                     {/* EXTRA CLASS DROPDOWN BUTTON */}
                     <div className="flex items-center gap-3 bg-white px-5 py-2.5 rounded-2xl shadow-sm border border-slate-200">
                         <label className="text-[10px] font-black uppercase text-indigo-600 tracking-widest">Extra Classes Today:</label>
                         <select 
                            value={extraClasses}
                     ...
                     <button
                       ...
                       className="inline-flex items-center gap-2 px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-lg shadow-emerald-200 transition-all active:scale-95"
                     >
                       <FileOutput size={14} />
              ...
              {loadingMatrix ? (
                <div className="flex justify-center py-20"><UniLifeLoader size="md" /></div>
              ) : !selectedCourse ? (
                ...
                <div className="bg-white rounded-[2.5rem] p-12 text-center border border-slate-200 text-slate-400 font-bold uppercase text-xs">No attendance records or students found for this course.</div>
              ) : (
                      ...
                      <thead>
                        <tr className="bg-slate-900 text-white">
                          ...
                          {combinedSessions.map((session) => (
                            <th key={session.key} className={`px-4 py-5 text-center text-[10px] font-black uppercase tracking-widest whitespace-nowrap border-l border-white/10 ${session.dateStr === todayStr ? 'bg-indigo-600 text-white' : ''}`}>
                              {session.display}
                            </th>
                          ))}
                        </tr>
                        ...
                        {matrixData.students.map((student, idx) => (
                          <tr key={student.studentId} className={idx % 2 === 0 ? "bg-white" : "bg-slate-50/50"}>
                            ...
                            {combinedSessions.map((session) => {
                              ...
                              const normalizedStatus = (status === 'attended' ? 'present' : (status === 'missed' ? 'absent' : status)) || "";

                              // STRICT LOGIC: ONLY today's cells are editable. Past is read-only.
                              const isEditable = isTeacher && session.dateStr === todayStr; 

                              return (
                                <td key={session.key} className={`px-3 py-3 border-b border-l border-slate-100 text-center align-middle ${session.dateStr === todayStr ? 'bg-indigo-50/20' : ''}`}>
                                  {isEditable ? (
                                     <select
                                        ...
                                        onChange={(e) => handleMarkAttendance(student.studentId, session, e.target.value)}
                                     >
                                        <option value="">-</option>
                                     ...
                                     normalizedStatus === "present" ? <span className="text-emerald-500 font-black text-sm">P</span> :
                                     normalizedStatus === "absent" ? <span className="text-rose-500 font-black text-sm">A</span> :
                                     normalizedStatus === "late" ? <span className="text-amber-500 font-black text-sm">L</span> :
