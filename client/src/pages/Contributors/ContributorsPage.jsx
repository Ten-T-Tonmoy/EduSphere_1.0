...
import { Trash2, Github, Linkedin, Sparkles, UserPlus, Edit3 } from 'lucide-react';
    ...
    <div className="min-h-screen lg:h-screen bg-slate-50 text-slate-900 overflow-y-auto lg:overflow-hidden font-sans flex flex-col relative">
      ...
      >
        <div className="inline-flex items-center gap-3 px-3 py-1 rounded-full bg-indigo-60 text-indigo-500 border border-indigo-600 mb-4">
          ...
          <span className="text-[11px] font-black uppercase tracking-wider">The Development Core</span>
        </div>
        <h1 className="text-4xl lg:text-6xl font-black tracking-tight text-slate-650 mb-3">
          Project <span className="text-indigo-600">Architects and Designers</span>
        </h1>
        <p className="text-slate-600 font-medium text-sm lg:text-base">Meet the engineers behind the UniLife Manager ecosystem.</p>
      ...
      <main className="relative z-10 flex-1 px-10 pb-10 flex items-center justify-center">
        <motion.div 
          variants={containerVariants}
          initial="hidden"
          whileInView="visible"
          viewport={{ once: true }}
          className="w-full max-w-[1400px] grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-7 h-full max-h-[600px]"
        >
          {members.map((member) => (
            <motion.div 
              key={member._id} 
              ...
              className="group relative flex flex-col h-full bg-white rounded-[2.5rem] border border-slate-200 shadow-sm hover:shadow-2xl hover:shadow-indigo-100 transition-all duration-500"
            >
              {/* Photo Container */}
              <div className="p-6 flex-shrink-none">
                <div className="relative rounded-[10rem] overflow-hidden aspect-square border border-slate-150 shadow-inner transform transition-shadow duration-900 group-hover:shadow-lg group-hover:shadow-indigo-500">
                  <img src={member.photo} className="w-full h-full object-cover grayscale-[30%] group-hover:grayscale-0 transition-all duration-700 cursor-pointer transform hover:scale-105" alt={member.name} />
                  ...
                  <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-2 translate-y-12 group-hover:translate-y-0 transition-transform duration-500">
                    <a href={member.github} target="_blank" rel="noreferrer" className="p-2.5 bg-white/90 backdrop-blur shadow-lg rounded-xl text-slate-900 hover:text-indigo-600 transition-colors"><Github size={18}/></a>
                    <a href={member.linkedin} target="_blank" rel="noreferrer" className="p-2.5 bg-white/90 backdrop-blur shadow-lg rounded-xl text-slate-900 hover:text-indigo-600 transition-colors"><Linkedin size={18}/></a>
                  </div>
                </div>
              </div>
              ...
              <div className="px-3 pb-4 pt-1 flex flex-col flex-1 text-center">
                <h1 className="text-xl font-black text-slate-900 leading-tight mb-1">{member.name}</h1>
                <p className="text-indigo-600 text-[14px] font-black uppercase tracking-widest mb-4">{member.role}</p>
                
                <div className="flex flex-wrap justify-center gap-2.5 mt-auto">
                  {member.contributions?.slice(0, 3).map((c, i) => (
                    <span key={i} className="text-[12px] font-bold uppercase bg-slate-200 text-slate-800 px-2 py-1 rounded-lg border border-slate-100">
                      {c}
                    </span>
                  ))}
                </div>
