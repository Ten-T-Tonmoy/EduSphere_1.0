...
const ContributorsPage = () => {
  ...
  return (
    <div className="min-h-screen bg-[#0f172a] text-white pb-20 overflow-hidden font-sans">
      ...
      <header className="relative pt-20 pb-12 text-center px-4">
        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/5 border border-white/10 backdrop-blur-md mb-6 animate-bounce">
          ...
          <span className="text-[10px] font-black uppercase tracking-[0.2em]">The Elite Squad</span>
        </div>
        ...
        <p className="text-gray-400 text-lg max-w-2xl mx-auto font-medium leading-relaxed">
          Crafting the future of university management with code, passion, and precision.
        </p>
          ...
          >
            ...
            <div className="relative bg-[#1e293b]/80 backdrop-blur-xl rounded-[2.5rem] p-8 border border-white/10 h-full">
              <div className="relative mb-8 rounded-[2rem] overflow-hidden aspect-square border-4 border-white/5 shadow-2xl">
                <img src={member.photo} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" alt={member.name} />
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent flex items-end justify-between p-6 opacity-0 group-hover:opacity-100 transition-all">
                  <div className="flex gap-3">
                    <a href={member.github} className="p-3 bg-white/10 rounded-full hover:bg-white hover:text-black transition-all"><Github size={18}/></a>
                    <a href={member.linkedin} className="p-3 bg-white/10 rounded-full hover:bg-white hover:text-black transition-all"><Linkedin size={18}/></a>
                  </div>
                </div>
              </div>

              <h3 className="text-2xl font-black mb-1">{member.name}</h3>
              <p className="text-indigo-400 text-sm font-bold uppercase tracking-widest mb-6">{member.role}</p>
              
              <div className="flex flex-wrap gap-2">
                {member.contributions?.map((c, i) => (
                  <span key={i} className="text-[9px] font-black uppercase bg-white/5 border border-white/10 px-3 py-1.5 rounded-lg text-gray-400">
                    {c}
                  </span>
                ))}
              </div>
      ...
      {showModal && <ContributorFormModal onClose={() => setShowModal(false)} onSave={fetchTeam} />}
    </div>
  );
};
