...
const ProfilePage = () => {
  ...
  useEffect(() => {
    const fetchGroups = async () => {
      try {
        ...
        const names =
          res.data.groups?.map((item) => item.group?.name).filter(Boolean) ||
          [];
  ...
  return (
    <div className="min-min-h-svh bg-slate-50/50 pb-16 font-sans">
            ...
            {isEditing ? (
              <>
                <button
                  ...
                  className="flex-1 sm:flex-none px-5 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl font-bold text-sm transition-colors flex items-center justify-center gap-2"
                >
                ...
                <button
                  ...
                  className="flex-1 sm:flex-none px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold text-sm transition-colors shadow-md flex items-center justify-center gap-2 disabled:opacity-50"
                >
                  {loading ? (
                    <UniLifeLoader size="sm" />
                  ) : (
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 sm:p-5 flex items-start gap-4 shadow-sm animate-in fade-in duration-500">
            ...
            <div>
              ...
              <p className="text-xs font-medium text-amber-700 mt-0.5">
                To maintain record integrity, personal details can only be
                changed once a month. You can edit your profile again in{" "}
                <strong className="text-amber-900">{daysRemaining} days</strong>
                .
              </p>
            </div>
          </div>
            ...
            <div className="space-y-5 flex-1">
              ...
              <div>
                <label className="text-xs font-bold text-slate-500 flex items-center gap-1.5 mb-1.5">
                  ...
                  {user?.role === "teacher" ? "Employee ID" : "Student ID"}
                </label>
              ...
              <div>
                <label className="text-xs font-bold text-slate-500 flex items-center gap-1.5 mb-1.5">
                  <Mail className="w-3.5 h-3.5" /> University Email
                </label>
                <div className="bg-slate-50 border border-slate-100 rounded-xl px-4 py-3">
              ...
              <div>
                <label className="text-xs font-bold text-slate-500 flex items-center gap-1.5 mb-1.5">
                  <Users className="w-3.5 h-3.5" /> Assigned Groups
                </label>
                <div className="bg-slate-50 border border-slate-100 rounded-xl px-4 py-3 flex flex-wrap gap-2">
                  {fetchedGroups.length === 0 ? (
                    ...
                    fetchedGroups.map((groupName, idx) => (
                      <span
                        key={idx}
                        className="bg-white border border-slate-200 text-slate-700 text-xs font-bold px-2.5 py-1 rounded-md shadow-sm"
                      >
                        {groupName}
                      </span>
                    ))
            ...
            {!isEditing && !isLocked && (
              <div className="mt-6 bg-slate-50 border border-slate-100 p-4 rounded-xl text-center">
                <p className="text-xs font-semibold text-slate-500">
                  You are eligible to update these details today. Remember,
                  saving changes will lock them for{" "}
