...
const GroupAttendance = ({ groupId, groupName, onClose }) => {
  ...
  useEffect(() => {
    const fetchGroups = async () => {
      try {
        ...
        const validGroups = res.data.groups?.map(g => g.group).filter(Boolean) || [];
  ...
  const fetchAttendance = async () => {
    ...
    try {
      setLoading(true);
      const res = await api.get(`/attendance/group/${activeGroupId}?courseId=${selectedCourse}`);
    ...
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl p-6 max-w-7xl w-full h-[90vh] flex flex-col shadow-2xl">
        ...
        <div className="mb-4 flex flex-wrap gap-3 items-center">
          ...
          >
            {groups.length === 0 && <option value={activeGroupId}>{activeGroupName}</option>}
            {groups.map(group => (
